...
function MapInstructions({ selectionState }: { selectionState: SelectionState }) {
  ...
  return (
    <div className="absolute top-3 left-3 right-12 sm:top-4 sm:left-4 sm:right-14 z-[1000]">
      <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200 px-3 py-2.5 sm:px-4 sm:py-3 flex items-center gap-2 sm:gap-3">
        <span className={`px-2 py-0.5 rounded text-xs font-bold text-white flex-shrink-0 ${current.color}`}>
          {current.step}
        </span>
        <p className="text-xs sm:text-sm text-slate-700 font-medium">{current.text}</p>
      </div>
    </div>
  );
}
